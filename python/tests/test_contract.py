...
from go2_energy.contract import read_csv, read_json, write_json
...
class ContractTests(unittest.TestCase):
    def test_schema_is_identical_to_public_browser_contract(self):
        installed = json.loads(
            files("go2_energy").joinpath("scenario.schema.json").read_text(encoding="utf-8")
        )
        self.assertEqual(
            installed,
    ...
    def test_cli_matches_library(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "go2_energy", "analyze", str(EXAMPLE), str(output)],
                capture_output=True,
            )
    ...
    def test_invalid_cli_preserves_last_report(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "go2_energy", "analyze", str(source), str(output)],
                capture_output=True,
            )
    ...
    def test_cli_refuses_input_overwrite(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "go2_energy", "analyze", str(source), str(source)],
                capture_output=True,
            )
            self.assertEqual(result.returncode, 1)
...
def parity_case(filename):
    def check(self):
        ...
        for key in ("availableWh", "reserveWh", "effectiveCapacityWh", "nominalWh", "marginWh"):
            close(self, actual[key], expected[key])
        for left, right in (
            ("upperWh", "highWh"),
            ("lowerWh", "lowWh"),
            ("durationS", "totalDurationS"),
            ("feasible", "feasibleWithinAssumptions"),
        ):
            close(self, actual[left], expected[right])
        for left, right in zip(actual["records"], expected["records"]):
            for key in ("id", "phase", "durationS", "energyWh", "cumulativeWh"):
                close(self, left[key], right[key])
